...
import bpdf.symbol.BooleanExpression;
import bpdf.symbol.BooleanValue;
...
public class BPDFConstraint {
    ...
    /** Right Function. Left function is considered to always be "i" */
    private Expression rFunc;
    /** Boolean guard toggling the constraint. Defaults to true (no guard) */
    private BooleanExpression guard = new BooleanValue(true);
    /** Set of the boolean parameters used in the constraint guard */
    private Set<String> params = new HashSet<String>();
    /** Boolean parameter periods */
    private Map<String, Product> periodMap = new HashMap<String, Product>();
    /** Boolean parameter values */
    private Map<String, List<Boolean>> valueMap = new HashMap<String, List<Boolean>>();
    /** Boolean value shift e.g. E(i+1) > D(i) becomes E(i) > D(i-1) */
    private int shift = 0;
    ...
    public void evaluateParam(String param, int n) {
        ...
        for (String p : params) {
            Product period = periodMap.get(p);
    ...
    public boolean evaluateGuard(int status) {
        for (String p : params) {
            List<Boolean> values = valueMap.get(p);
            guard.setValue(p, values.get(status));
        }
        return guard.isSet();
    }
    ...
    public void setParam(String param, boolean value) {
        if (params.contains(param)) {
            Product period = periodMap.get(param);
            List<Boolean> values = valueMap.get(param);
